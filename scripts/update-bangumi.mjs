...
const REQUEST_HEADERS = {
	"User-Agent":
		"flyinglandlord.github.io/1.0 (https://github.com/flyinglandlord/flyinglandlord.github.io)",
};
...
async function main() {
	...
	let cachedAnimeList = [];
	try {
		cachedAnimeList = JSON.parse(await fs.readFile(OUTPUT_FILE, "utf-8"));
		if (!Array.isArray(cachedAnimeList)) cachedAnimeList = [];
	} catch {
		cachedAnimeList = [];
	}
	...
	if (hadFetchFailure) {
		console.warn(
			cachedAnimeList.length > 0
				? `Bangumi API was unavailable. Keeping ${cachedAnimeList.length} cached entries.`
				: "Bangumi API was unavailable and no cache exists; leaving the anime list empty.",
		);
		return;
	}
